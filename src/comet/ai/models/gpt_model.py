...
from src.comet.config.env import GPT_DEFAULT_MAX_TOKENS, GPT_DEFAULT_TEMPERATURE
...
class GPTModelParams(ModelParamsBase):
    ...
    max_tokens : int
        ...
        completion. Must be between 1 and GPT_DEFAULT_MAX_TOKENS.
    temperature : float
        The temperature parameter for sampling, controlling randomness.
        Must be between 0.0 and GPT_DEFAULT_TEMPERATURE.
    top_p : float
    ...
    def max_tokens(self, value: int) -> None:
        ...
        if not 1 <= value <= GPT_DEFAULT_MAX_TOKENS:
            msg = f"The max_tokens must be between 1 and {GPT_DEFAULT_MAX_TOKENS}."
    ...
    def temperature(self, value: float) -> None:
        ...
        if not 0.0 <= value <= GPT_DEFAULT_TEMPERATURE:
            msg = f"The temperature must be between 0.0 and {GPT_DEFAULT_TEMPERATURE}."
            raise ValueError(msg)

...
from src.comet.config.env import CLAUDE_DEFAULT_MAX_TOKENS, CLAUDE_DEFAULT_TEMPERATURE
...
class ClaudeModelParams(ModelParamsBase):
    ...
    def max_tokens(self, value: int) -> None:
        ...
        if not 1 <= value <= CLAUDE_DEFAULT_MAX_TOKENS:
            msg = f"The max_tokens must be between 1 and {CLAUDE_DEFAULT_MAX_TOKENS}."
    ...
    def temperature(self, value: float) -> None:
        ...
        if not (0.0 <= value <= CLAUDE_DEFAULT_TEMPERATURE):
            msg = f"Claude's temperature must be between 0.0 and {CLAUDE_DEFAULT_TEMPERATURE}."
            raise ValueError(msg)
